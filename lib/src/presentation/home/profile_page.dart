...
class _ProfilePageState extends State<ProfilePage> {
  ...
  String _key(String field) => 'profile_${field}_$_userPrefix';
  ...
  Future<void> _loadProfileData() async {
    ...
    setState(() {
      _firstNameController.text = prefs.getString(_key('firstName')) ?? '';
      _lastNameController.text = prefs.getString(_key('lastName')) ?? '';
      _emailController.text = prefs.getString(_key('email')) ?? '';
      _phoneController.text = prefs.getString(_key('phone')) ?? '';
      _birthDateController.text = prefs.getString(_key('birthDate')) ?? '';
      _baptismDateController.text = prefs.getString(_key('baptismDate')) ?? '';
      _churchLocationController.text = prefs.getString(_key('churchLocation')) ?? '';
      _profileImagePath = prefs.getString(_key('imagePath'));
      _selectedDepartments = prefs.getStringList(_key('departments')) ?? [];
    });
  }
  ...
  Future<void> _pickImage() async {
    ...
    if (pickedFile != null) {
      ...
      final fileName = 'profile_$_userPrefix.jpg';
  ...
  Widget _buildDepartmentSelector() {
    return Column(
      ...
      children: [
        Text(
          'Departamentele în care slujești',
          style: GoogleFonts.inter(
            fontSize: 14,
            fontWeight: FontWeight.w600,
            color: beige,
          ...
          return Padding(
            ...
            child: InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: isSelected
                      ? navy.withValues(alpha: 0.4)
                      : navy.withValues(alpha: 0.15),
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(
                    color: isSelected ? navy : navy.withValues(alpha: 0.3),
                  ),
                ),
                child: Row(
                  children: [
                    Icon(
                      ...
                      color: isSelected ? lightGrey : beige.withValues(alpha: 0.5),
                      size: 22,
                    ),
                    ...
                    Expanded(
                      child: Text(
                        ...
                        style: GoogleFonts.inter(
                          fontSize: 15,
                          color: isSelected ? lightGrey : beige,
  ...
  Future<void> _saveProfile() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_key('firstName'), _firstNameController.text);
    await prefs.setString(_key('lastName'), _lastNameController.text);
    await prefs.setString(_key('email'), _emailController.text);
    await prefs.setString(_key('phone'), _phoneController.text);
    await prefs.setString(_key('birthDate'), _birthDateController.text);
    await prefs.setString(_key('baptismDate'), _baptismDateController.text);
    await prefs.setString(_key('churchLocation'), _churchLocationController.text);
    if (_profileImagePath != null) {
      await prefs.setString(_key('imagePath'), _profileImagePath!);
    }
    await prefs.setStringList(_key('departments'), _selectedDepartments);
